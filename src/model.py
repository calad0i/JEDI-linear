...
def get_gnn(conf, uq1: bool = False):
    ...
    with (
        ...
        x = QEinsumDenseBatchnorm('bnc,cC->bnC', (N, 64), bias_axes='C', activation='relu', iq_conf=iq_conf)(inp)
        s = QEinsumDenseBatchnorm('bnc,cC->bnC', (N, 64), bias_axes='C', activation='relu', )(x)
        d = QEinsumDenseBatchnorm('bnc,cC->bnC', (1, 64), bias_axes='C', activation='relu', )(QSum(axes=1, scale=pool_scale, keepdims=True)(x))
        x = QAdd()([s, d])
...
def get_model(conf):
    scope0 = QuantizerConfigScope(default_q_type='kbi', b0=conf.model.init_bw_k, overflow_mode='wrap', i0=0, fr=MonoL1(conf.model.k_bw_l1_reg), ir=MonoL1(conf.model.k_bw_l1_reg))
    scope1 = QuantizerConfigScope(default_q_type='kif', place='datalane', overflow_mode='wrap', f0=conf.model.init_bw_a, fr=MonoL1(conf.model.a_bw_l1_reg), ic=MinMax(0, 12))
