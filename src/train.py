...
def train_hgq(model: keras.Model, X, Y, Xs, Ys, conf):
    ...
    pbar = PBar(metric='loss: {loss:.2f}/{val_loss:.2f} - acc: {accuracy:.2%}/{val_accuracy:.2%} - lr:{learning_rate:.2e} - beta: {beta:.2e}')
    ...
    dataset_train = Dataset(X_train, Y_train, batch_size=batch_size, drop_last=True, device='gpu:0')
    dataset_val = Dataset(X_val, Y_val, batch_size=batch_size, drop_last=True, device='gpu:0')

    model.fit(dataset_train, epochs=conf.train.epochs, validation_data=dataset_val, callbacks=callbacks, verbose=0)  # type: ignore
